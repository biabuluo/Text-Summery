...
class Decoder(nn.Module):
    def __init__(self, vocab, config, attention, embed_layer):
        ...
        self.isCoverage = config.coverage
    ...
    def forward(self, dec_input, s, enc_output, text_lengths, coverage_vector=None):
        ...
        if self.isCoverage:
            ...
            a = self.attention(embedded, enc_output, text_lengths)
        a = a.unsqueeze(1)
        ...
        if self.isCoverage:
            ...
            return self.softmax(dec_output), dec_hidden, a.squeeze(1), p_gen
...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        self.device = config.device
        # 用于计算没有coverage机制的attention 分布
        self.attn = nn.Linear(config.hidden_size * 2 + config.emb_size, config.hidden_size)
        self.v = nn.Linear(config.hidden_size, 1)
        self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(self, dec_input, enc_output, text_lengths):
        ...
        x = torch.tanh(self.attn(torch.cat([enc_output, dec_input], dim=2)))
        attention = self.v(x).squeeze(-1)  # [batch_size, seq_len, ]
        # 对每个句长做掩码：不计入填充字段mask = [batch_size, seq_len]
        max_len = enc_output.shape[1]
        mask = torch.arange(max_len).expand(text_lengths.shape[0], max_len) >= text_lengths.unsqueeze(1)
...
class PGN(nn.Module):
    def __init__(self, vocab, config):
        ...
        self.isCoverage = config.coverage
        if self.isCoverage:
            attention = Attention_with_coverage(config)
        else:
        ...
        self.decoder = Decoder(vocab, config, attention, self.embed_layer)
    ...
    def forward(self, src, tgt, src_lengths, len_oovs, teacher_forcing_ratio=0.5):
        ...
        for t in range(tgt_len-1):
            ...
            if self.isCoverage:
                ...
                dec_output, s, a, p_gen = self.decoder(dec_input,
                                                       s,
                                                       enc_output,
                                                       src_lengths)
