...
# 模型验证过程
def evaluate(model, val_dataloader, loss_fn, device, config):
    val_loss = []
    model.eval()
    with torch.no_grad():
        val_loss = []
        for i, (text, text_len, title_in, title_out, oovs_list, len_oovs) in enumerate(val_dataloader):
            text = text.to(device)
            title_in = title_in.to(device)
            title_out = title_out.to(device)
            title_pred, a, coverage_vector = model(text, title_in, text_len, len_oovs)
...
def train(model, train_dataloader, val_dataloader, loss_fn, optimizer, config):
    ...
    for epoch in range(config.epochs):
        ...
        for i, (text, text_len, title_in, title_out, oovs, len_oovs) in enumerate(train_dataloader):
            ...
            if config.coverage:
                # 计算 coverage loss
                c_t = torch.min(a, coverage_vector)
                cov_loss = torch.mean(torch.sum(c_t, dim=1))
                # 计算整体 loss
                loss += config.cov_lambda * cov_loss
        ...
        # 每个 epoch 结束，验证模型的精度
        avg_val_loss = evaluate(model, val_dataloader, loss_fn, config.device, config)
        print_logger()
        print(
        ...
        if epoch == 0 or avg_val_loss < min_val_loss:
            model_path = config.saved_model_path + 'model.pt'
            # 保存模型
